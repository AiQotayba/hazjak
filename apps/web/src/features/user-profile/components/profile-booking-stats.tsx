  ...
  return (
    <section className="mb-4">
      ...
      {stats.nextBooking ? (
        <Link
          href="/user/bookings"
          className="mt-3 block rounded-2xl bg-gradient-to-l from-primary/8 to-section-alt p-3 shadow-soft transition-colors hover:shadow-card"
        >
          <p className="text-[10px] font-bold text-primary mb-1.5">أقرب حجز لك</p>
          <p className="text-sm font-bold text-heading truncate">{stats.nextBooking.stadium.name}</p>
...
function StatTile({
  ...
  return (
    <div className="rounded-2xl bg-card p-3 shadow-soft">
      <div className={cn("inline-flex h-8 w-8 items-center justify-center rounded-xl mb-2", accent)}>
        <Icon className="h-4 w-4" />
      </div>
      <p className="font-display text-xl font-bold text-heading leading-none">{value}</p>
